...
const execFileAsync$3 = node_util.promisify(childProcess$1.execFile);
...
const execFileAsync$2 = node_util.promisify(childProcess$1.execFile);
...
const execFileAsync$1 = node_util.promisify(childProcess$1.execFile);
...
const execFileAsync = node_util.promisify(childProcess$1.execFile);
...
const __dirname$2 = path$2.dirname(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('cli.js', document.baseURI).href))));
const localXdgOpenPath = path$2.join(__dirname$2, 'xdg-open');
...
const baseOpen = async options => {
	...
	if (platform === 'darwin') {
		...
		if (app) {
			...
			const useSystemXdgOpen = process$2.versions.electron
	...
	const subprocess = childProcess$1.spawn(command, cliArguments, childProcessOptions);
...
const __filename$1 = url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('cli.js', document.baseURI).href)));
...
const publicDir = `${__dirname$1}/dist`;

const calcUrl = (fileName) => {

  return isImg(fileName) ? process$2.cwd() + fileName : join(publicDir, fileName)
};

const getmimeType = (id) => {
  let mimeType;
  const ext = path$1.extname(id);
  switch (ext) {
    case '.html':
      mimeType = 'text/html';
      break;
...
function createHostServer(json) {
  ...
  router.get(
    ...
    eventHandler(async (event) => {
      ...
      const htmlPath = path$1.join(__dirname$1, '/dist/index.html');

      const htmlContent = await fs$1.readFileSync(htmlPath, 'utf-8');
      return htmlContent;
    })
  );
  app.use(
    defineEventHandler((event) => {
      return serveStatic(event, {
        getContents: (id) => {
          ...
          return fs$3.readFile(calcUrl(id))
        },
        getMeta: async (id) => {

          if (id == 'json') return json
          let stats = await fs$3.stat(calcUrl(id)).catch(() => { });
  ...
  return http.createServer(toNodeListener(app));
}
